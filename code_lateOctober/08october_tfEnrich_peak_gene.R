...
data("mouse_pwms_v1")
dt <- fread(paste0("zcat < ", "../peak_gene/Peak-to-gene_perPeak_Pearson_noStromal.txt.gz"))

keepPeaks <- fread("../data/immgen_good_peaks.txt", header = FALSE)[[1]]
kidx <- as.numeric(gsub("ImmGenATAC1219.peak_", "", keepPeaks))
peaks_g <- makeGRangesFromDataFrame(data.frame(fread("../data/ImmGenATAC1219.peak.bed", header = FALSE))[kidx,],
                                    seqnames.field = "V1", start.field = "V2", end.field = "V3", keep.extra.columns = TRUE)
matchm <- motifmatchr::matchMotifs(mouse_pwms_v1, peaks_g, genome = BSgenome.Mmusculus.UCSC.mm10)
mdf <- assays(matchm)[["motifMatches"]]

neg <- mcols(peaks_g)[["V4"]] %in% (dt[dt[["V4"]] > 4 & dt[["V5"]] < 0,])[["V1"]]
pos <- mcols(peaks_g)[["V4"]] %in% (dt[dt[["V4"]] > 4 & dt[["V5"]] > 0,])[["V1"]]

sum(neg)
sum(pos)

negEnrich <- sapply(1:dim(mdf)[2], function(i){
  tt <- data.frame(neg, mdf[,i])
  print(i)
  ft <- fisher.test(matrix(c( sum(tt[,1] & tt[,2]), sum(!tt[,1] & tt[,2]), sum(tt[,1] & !tt[,2]), sum(!tt[,1] & !tt[,2])),ncol =2))
  return(c(ft$estimate, ft$p.value))
}) %>% t()

plotdf <- data.frame(stat = sign(log2(negEnrich[,1]))* -1*log10(negEnrich[,2]),
                     logp = -1*log10(negEnrich[,2]),
                     tf = colnames(mdf), sign = sign(log2(negEnrich[,1])))
plotdf$sign <- ifelse(plotdf$sign == 1, "Enriched in Repressive Peak", "Depleted")
plotdf <- plotdf[order(plotdf$stat, decreasing = TRUE),]
plotdf$rank <- 1:dim(plotdf)[1]

 ggplot() + pretty_plot() + 
    geom_bar(data =plotdf, aes(x=rank, y=stat, fill = sign),stat = "identity") +
    scale_fill_manual(values=c("black", "red")) + 
    theme( legend.position="bottom")+labs(fill = "Motif Direction") +
