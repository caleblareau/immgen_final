...
doAll <- function(farr){
  ...
  write.table(plotdf, file = paste0("output/varianceComponent_promoterTFenrich.", as.character(farr), "bp.results.txt"),
  ...
  p1 <- ggplot() + pretty_plot() + 
    ...
    scale_fill_manual(values=c("dodgerblue3", "green3", "grey")) + 
  ...
  ggsave(p1,  file = paste0("output/varianceComponent_promoterTFenrich.", as.character(farr), "bp.pdf"))
}
...
doKmer <- function(farr){
  ...
  write.table(plotdf, file = paste0("output/varianceComponent_promoterKMERenrich.", as.character(farr), "bp.results.txt"),
              row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE)
  ...
  ggsave(p1,  file = paste0("output/varianceComponent_promoterKMERenrich.", as.character(farr), "bp.pdf"))
}
