...
# Import Sam data
mat <- (readRDS("data/euclideanDist_myeloid_withProgenitors.rds"))
...
vals <- sapply(1:dim(devmodel)[1], function(i){
  Y <- (devmodel[i,])
  mod <- lmm.aireml(Y = scale(Y), K = list(related), verbose = FALSE)
...
write.table(vdf2, file = "myeloid_lineageDefined.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
