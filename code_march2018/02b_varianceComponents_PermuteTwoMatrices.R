...
boom <- rbind(promoters, all_distal)
idx <- sample(1:dim(boom)[1])
P <- cor(boom[1:dim(promoters)[1],])
D <- cor(boom[(dim(promoters)[1]+1):dim(boom)[1],])
...
cowplot::ggsave(p1, file = "figures/newVarianceComponent_permute_2randomMatrices.pdf", height = 2, width = 4)
